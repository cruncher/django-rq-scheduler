...
class ScheduledJobAdmin(QueueMixin, admin.ModelAdmin):
    list_display = ("name", "job_id", "is_scheduled", "scheduled_time", "enabled")
    list_filter = ("enabled",)
    ...
    fieldsets = (
        ...
        (
            _("Scheduling"),
            {
                "fields": ("scheduled_time", "timeout", "result_ttl"),
            },
        ),
    )
...
class CronJobAdmin(QueueMixin, admin.ModelAdmin):
    list_display = ("name", "job_id", "is_scheduled", "cron_string", "enabled")
    list_filter = ("enabled",)
    list_editable = ("enabled",)
